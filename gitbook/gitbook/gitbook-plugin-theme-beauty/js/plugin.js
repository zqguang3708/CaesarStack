...
function handlerH3Toc(config, count, header, tocs, pageLevel, modifyHeader) {
    ...
    var id = header.attr('id');
    var level = ''; //层级
    header.html('<span name="' + id + '" >' + title + '</span>');
    if (tocs.length <= 0) {
        //一级节点为空时，生成一个空的一级节点，让二级节点附带在这个上面
        if (config.showLevel) {
            count.h1 += 1;
...
function buildGoTop(tocs) {
    ...
    if (config.showGoTop && tocs && tocs.length > 0) {
        if (tocs[0].name) {
            ...
            var tocH2 = tocs[0].children;
            if (tocH2[0].name) {
                html = "<a href='#" + tocH2[0].url + "' id='anchorNavigationExGoTop'><i class='fa fa-arrow-up'></i></a>";
            } else {
                var tocH3 = tocH2[0].children;
                if (tocH3[0].name) {
                    html = "<a href='#" + tocH3[0].url + "' id='anchorNavigationExGoTop'><i class='fa fa-arrow-up'></i></a>";
                }
            }
        }
    }
